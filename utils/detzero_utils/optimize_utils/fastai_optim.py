...
class OptimWrapper2():
    ...
    def __init__(self, opt, lr_max, wd, true_wd: bool = False, bn_wd: bool = True):
        self.opt, self.true_wd, self.bn_wd = opt, true_wd, bn_wd
        self.opt_keys = list(self.opt.param_groups[0].keys())
        self.opt_keys.remove('params')
        self.read_defaults()
    ...
    @classmethod
    def create_from_params(cls, lr_max, opt_func, params, **kwargs):
        opt = opt_func(params)
        opt = cls(opt, lr_max, **kwargs)
    ...
    @lr.setter
    def lr(self, val: float) -> None:
        self._lr = [val*lr_mult for lr_mult in self._lr_mult]
        self._lr = self.set_val('lr', self._lr)
    ...
    def read_defaults(self) -> None:
        ...
        if 'betas' in self.opt_keys: self._mom, self._beta = self.read_val('betas')
        if 'weight_decay' in self.opt_keys: self._wd = self.read_val('weight_decay')
        if 'lr_mult' in self.opt_keys: self._lr_mult = self.read_val('lr_mult')
        else:
    ...
    def set_val(self, key: str, val, bn_groups: bool = True):
        "Set `val` inside the optimizer dictionary at `key`."
        if is_tuple(val): val = [(v1, v2) for v1, v2 in zip(*val)]
        for v, pg in zip(val, self.opt.param_groups):
            pg[key] = v
        return val
