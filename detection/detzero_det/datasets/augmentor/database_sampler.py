...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        for db_info_path in sampler_cfg.DB_INFO_PATH:
            db_info_path = os.path.join(os.path.abspath(self.root_path), db_info_path)
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        if points.shape[1] >= 7:
            obj_points = np.pad(obj_points, ((0, 0), (0, 1)), mode='constant', constant_values=0)

            current_points_mask = (points[:, -1] == 0)
            history_points = points[~current_points_mask]
            current_points = box_utils.remove_points_in_boxes3d(points[current_points_mask], large_sampled_gt_boxes)
            points = np.concatenate((current_points, history_points), axis=0)
