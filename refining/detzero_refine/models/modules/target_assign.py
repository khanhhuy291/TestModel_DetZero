...
class TargetAssigner(object):
    ...
    def encode_torch(self, data_dict):
        ...
        if self.mode == 'confidence':
            raw_iou = data_dict['iou'].reshape(-1)
            valid = torch.isfinite(raw_iou) & (raw_iou >= 0)
            iou = torch.clamp(raw_iou, 0, 1)
            gt_label = torch.zeros_like(iou)
            mask = torch.zeros_like(iou)

            neg_idx = valid & (iou < self.score_thresh[0])
            mask[neg_idx] = 1
            
            pos_idx = valid & (iou >= self.score_thresh[1])
            mask[pos_idx] = 1
            gt_label[pos_idx] = 1
